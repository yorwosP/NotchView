...
public class Notch:UIView {
    ...
    // class method
    public static var isNotchAvailable: Bool {
        if let _ = Name(UIDevice.modelName){
            return true
        }else{
            return false
        }
...
// MARK: - properties
    public var name:Name = .iPhone13Mini
    

    
    /// set/get the cornerRadius of the layer
    ...
    private func setupView(){
        // I don't think we will ever hit the following part (already checked in init) 
        let currentName = UIDevice.modelName
        guard let name = Name(currentName) else {
            return
